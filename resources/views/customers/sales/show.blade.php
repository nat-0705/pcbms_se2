    ...
    <div class="col-md-10">
        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                <div class="row clearfix justtify-content-md-center">
                    ...
                    <div class="col-md-3">
                        <div class="no-padding no-margin"> <strong>Date:</strong> {{ $sales->date }}</div>
                        <div class="no-padding no-margin"> <strong>Bank No:</strong> {{ $sales->bank_no }}</div>
                    </div>
                </div>
            </div>
                    ...
                    <tbody>
                        @foreach ($sales->items as $key => $item)
                            <tr>
                                ...
                                <td class="text-right">
                                    <form method="post" action="{{ route('customers.sales.items.destroy',[$customers->id, $sales->id, $item->id]) }}">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            ...
                            <th class="text-right">{{ $totalSales = $sales->items()->sum('total') }}</th>
                        </tr>
                        <tr>
                            <th></th>
                            <th>
                                <button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#receiptForInvoiceModal">
                                    <i class="fa fa-plus"></i> Add Receipts
                                </button>
                            </th>
                            <th colspan="2" class="text-right">Paid: </th>
                            <th class="text-right">{{ $totalReceipts = $sales->receipts->sum('amount') }}</th>
                            <th></th>
                        </tr>
                        <tr>
                            <th colspan="4" class="text-right">Due: </th>
                            <th class="text-right">{{ $totalSales - $totalReceipts }}</th>
                            <th></th>
                        </tr>
                </table>
            </div>
        ...
        <div class="modal fade" id="productsModal" tabindex="-1" role="dialog" aria-labelledby="productsModalTitle" aria-hidden="true">
            <div class="modal-dialog" role="document">
                {!! Form::open(['route' => ['customers.sales.items.store', [$customers->id, $sales->id]],  'method' => 'post']) !!}
        ...
        <!-- Modal Sale Receipt -->
        <div class="modal fade" id="receiptForInvoiceModal" tabindex="-1" role="dialog" aria-labelledby="receiptForInvoiceModalTitle" aria-hidden="true">
            <div class="modal-dialog" role="document">
                {!! Form::open(['route' => ['customers.receipts.store', $customers->id, $sales->id],  'method' => 'post']) !!}
                    <div class="modal-content">
                        <div class="modal-header">
